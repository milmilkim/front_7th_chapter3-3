        ...
        {comments?.map((comment, index) => (
          <div key={comment.id} className="flex items-center justify-between text-sm border-b pb-1">
            <div className="flex items-center space-x-2 overflow-hidden">
              {comment.user && <span className="font-medium truncate">{comment.user.username}:</span>}
              <span className="truncate">{highlightTextUtil(comment.body, searchQuery)}</span>
            </div>
            <div className="flex items-center space-x-1">
              <LikeCommentButton commentId={comment.id} commentIndex={index} postId={postId} likes={comment.likes} />
              <Button variant="ghost" size="sm" onClick={() => handleEditComment(comment)}>
                <Edit2 className="w-3 h-3" />
              </Button>
              <DeleteCommentButton commentId={comment.id} commentIndex={index} postId={postId} />
            </div>
          </div>
        ))}
      </div>
